...
export default function InvoiceGenerator() {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    { id: "1", description: "Professional Services", quantity: 10, unitPrice: 150, phone: "", activationDate: new Date().toISOString().split("T")[0] },
    { id: "2", description: "Consulting Hours", quantity: 5, unitPrice: 200, phone: "", activationDate: new Date().toISOString().split("T")[0] },
  ])
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 md:p-12">
      ...
      {/* Invoice Container */}
      <div ref={invoiceRef} className="bg-white rounded-lg shadow-lg border border-border overflow-hidden">
        {/* Header Section */}
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="text-right space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-end items-center gap-3">
                  <span className="text-sm font-medium text-primary-foreground/80">Fecha de Facturación</span>
                  {isExporting ? (
                    <span className="text-primary-foreground w-40 text-right">{activationDate}</span>
                  ) : (
                    <Input
                      type="date"
                  ...
                  <tr key={item.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    <td className="py-4 px-2" colSpan={3}>
                      <div className="space-y-1">
                        {isExporting ? (
                          <div className="font-medium py-1">{item.description}</div>
                        ) : (
                          <Input
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          {isExporting ? (
                            ...
                            <Input
                              type="tel"
                              value={item.phone}
                              onChange={(e) => updateLineItem(item.id, "phone", e.target.value)}
                              placeholder="Requesting phone"
                              className="border-none p-0 h-auto focus-visible:ring-0 focus-visible:ring-offset-0 w-40 text-muted-foreground"
                            />
                          )}
                        </div>
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <span className="text-xs">Activation Date:</span>
                          {isExporting ? (
                            <span>{item.activationDate}</span>
                          ) : (
                            <Input
                              type="date"
                              value={item.activationDate}
                              onChange={(e) => updateLineItem(item.id, "activationDate", e.target.value)}
                              className="border-none p-0 h-auto focus-visible:ring-0 focus-visible:ring-offset-0 w-40 text-muted-foreground"
                            />
                          )}
                        </div>
